...
def simulate(
    ...
    if sc_key not in SCENARIOS:
        raise ValueError(
            f"Unknown scenario '{sc_key}'. Choose: {list(SCENARIOS.keys())}"
        )
...
Examples:
  ...
  python simulate_hvac_traffic.py --n 10 --dry-run
    ...
    parser.add_argument(
        "--n", type=int, default=200, help="Number of requests (default: 200)"
    )
    parser.add_argument(
        "--scenario", choices=["normal", "heatwave", "fault"], default="normal"
    )
    parser.add_argument(
        "--drift",
        action="store_true",
        help="Use out-of-distribution data (tests drift alerts)",
    )
    parser.add_argument(
        "--url", default=DEFAULT_URL, help=f"API endpoint (default: {DEFAULT_URL})"
    )
    parser.add_argument(
        "--model",
        default=DEFAULT_MODEL,
        help="RL model: qlearning or sarsa (default: qlearning)",
    )
    parser.add_argument(
        "--delay",
        type=float,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Build payloads but do not send HTTP"
    )
