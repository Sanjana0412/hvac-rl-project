...
from env import HVACEnv, QLearningAgent, SARSAAgent, ACTION_LABELS
from env import TEMP_IN_BINS, TEMP_OUT_BINS, HOUR_BINS
...
app = FastAPI(title="HVAC RL API", version="1.0.0",
...
def load_model(name):
    ...
    if not path or not os.path.isfile(path):
        raise HTTPException(404, f"Model '{name}' not found. Available: {available_models()}")
...
def discretise(indoor_temp, outdoor_temp, occupancy, hour):
    return (int(np.digitize(indoor_temp,  TEMP_IN_BINS)),
...
def health():
    return {"status": "ok", "timestamp": datetime.utcnow().isoformat(),
            "models_loaded": list(_cache.keys()), "models_available": available_models()}


@app.get("/models")
def get_models():
    return [{"name": k, "available": os.path.isfile(v), "path": v}
...
def predict(req: PredictRequest):
    ...
    state = discretise(req.indoor_temp, req.outdoor_temp, req.occupancy, req.hour_of_day)
    q_vals = agent.q_table[state].tolist()
    action = int(np.argmax(q_vals))
    resp   = {"action": action, "action_label": ACTION_LABELS[action],
               "state": list(state), "q_values": q_vals, "model": req.model}
