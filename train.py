...
import os, sys, json, csv, argparse, time, uuid
import numpy as np
import yaml

from env import HVACEnv, QLearningAgent, SARSAAgent, build_agent, ACTION_LABELS
...
def plot_comparison(results, plot_dir):
    try:
        ...
        metrics = [("avg_reward", "Reward"), ("avg_energy", "Energy kWh"), ("avg_carbon", "Carbon kg")]
...
def train_agent(agent_type, cfg, mlflow_parent_id=None):
    ...
    agent     = build_agent(agent_type, lr=ag["learning_rate"],
    ...
    if MLFLOW:
        ...
        mlrun = mlflow.start_run(run_name=f"{agent_type}_{run_id}",
                                 nested=mlflow_parent_id is not None)
    ...
    for ep in range(1, n_eps + 1):
        ...
        rewards.append(ep_r); energies.append(env.sim.energy_used)
        ...
        if MLFLOW and mlrun:
            mlflow.log_metrics({"reward": ep_r, "energy": env.sim.energy_used,
                                "carbon": env.sim.carbon_emitted,
                                "epsilon": agent.epsilon}, step=ep)

        if ep % max(1, n_eps // 10) == 0:
            print(f"  Ep {ep:>4}/{n_eps} | reward={np.mean(rewards[-20:]):.2f} "
                  f"| energy={np.mean(energies[-20:]):.2f} | ε={agent.epsilon:.3f}")

    elapsed = time.time() - t0
    summary = {"run_id": run_id, "agent_type": agent_type, "episodes": n_eps,
...
def evaluate_agent(agent, label, n_episodes=100, seeds=None):
    ...
    for seed in seeds:
        ...
        for _ in range(n_episodes):
            state = env.reset(); done = False; total = 0.0
            while not done:
...
def run_evaluation(cfg):
    ...
    class FixedBaseline:
        def choose_action(self, state, training=False): return 2  # always COOL
    ...
    for agent_type in ("qlearning", "sarsa"):
        ...
        if not os.path.isfile(path):
            print(f"  [Skip] {path} not found — run training first"); continue
    ...
    print("-" * 72)
    for r in results:
        print(f"{r['label']:<20} {r['avg_reward']:>10.3f} {r['std_reward']:>8.3f} "
...
def run_tuning(agent_type, cfg):
    grid = cfg.get("tuning", {}).get("param_grid", {
    ...
    for atype in agents:
        ...
        for combo in combos:
            ...
            tune_cfg = {**cfg, "agent": {**cfg["agent"], **params,
                                          "epsilon_start": 1.0, "epsilon_min": 0.05}}
            tune_cfg["training"] = {**cfg["training"], "episodes": n_eps}

            env   = HVACEnv(seed=cfg["simulator"]["seed"])
            agent = build_agent(atype, lr=params["learning_rate"],
            ...
            for _ in range(n_eps):
                state = env.reset(); done = False; ep_r = 0.0; steps = 0
                if is_sarsa: action = agent.choose_action(state)
                while not done:
                    if not is_sarsa: action = agent.choose_action(state)
                    ns, r, done, _ = env.step(action)
                    if is_sarsa:
                        na = agent.choose_action(ns)
                        agent.update(state, action, r, ns, na, done); action = na
                    else:
                        agent.update(state, action, r, ns, done)
                    state = ns; ep_r += r; steps += 1
                agent.decay_epsilon()
        ...
        if best["score"] > overall_best["score"]:
            overall_best = {"score": best["score"], "agent": atype, "params": best["params"]}
...
def register_models(cfg):
    if not MLFLOW:
        print("[Skip] mlflow not installed"); return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent",    choices=["qlearning", "sarsa", "both"], default="qlearning")
    ...
    parser.add_argument("--register", action="store_true", help="Register models in MLflow registry")
