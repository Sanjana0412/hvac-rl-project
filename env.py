...
class HVACSimulator:
    ...
    def step(self, action):
        ...
        delta = (0.12 * (self.outdoor_temp - self.indoor_temp)
    ...
    def _state(self):
        return {"indoor_temp": round(self.indoor_temp, 2),
                "outdoor_temp": round(self.outdoor_temp, 2),
                "occupancy": self.occupancy, "hour_of_day": self.hour}
...
class HVACEnv:
    ...
    def step(self, action):
        ...
        info   = {**raw, "energy_step": energy, "carbon_step": carbon,
    ...
    def _disc(self, raw):
        return (int(np.digitize(raw["indoor_temp"],  TEMP_IN_BINS)),
    ...
    def _reward(self, t_in, occ, energy, carbon):
        ...
        cost = self.w_energy * energy + self.w_comfort * comfort_pen + self.w_carbon * carbon
        return round(float(np.clip(np.tanh(-cost / 5.0) * 100.0, -100.0, 100.0)), 4)


# ── Base Agent ───────────────────────────────────────────────
class _BaseAgent:
    def __init__(self, lr=0.1, discount=0.95, eps_start=1.0, eps_min=0.05, eps_decay=0.995):
        self.lr        = lr
        self.gamma     = discount
    ...
    def save(self, path):
        ...
        with open(path, "wb") as f:
            pickle.dump({"q_table": self.q_table, "agent_type": self.agent_type,
                         "lr": self.lr, "gamma": self.gamma,
                         "epsilon": self.epsilon, "eps_min": self.eps_min,
                         "eps_decay": self.eps_decay}, f)
...
class SARSAAgent(_BaseAgent):
    ...
    def update(self, state, action, reward, next_state, next_action, done):
        target = reward + (0 if done else self.gamma * self.q_table[next_state][next_action])
        self.q_table[state][action] += self.lr * (target - self.q_table[state][action])
